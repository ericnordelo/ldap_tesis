...
Es muy similar al módulo sigenu-client en su estructura. También está compuesto de un archivo init con el código fuente, y un archi de configuración nombrado config.yml, en el cual se especifica la dirección de la base de datos fuente, la dirección del servidor sqlserver en que se monta la misma, para la adecuada extracción de la información, y el esquema de los datos a consumir, que no es más que una lista de pares ordenados donde el primero representa el nombre del atributo a generar en el archivo LDIF final, y el segundo el número de la columna en la que aparece.
